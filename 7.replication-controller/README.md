...
```bash
$ kubectl create ns replication-controller
```
### 2. Create and Get replication-controller Object
```bash
$ kubectl get -f rc.yml -n replication-controller
$ kubectl get rc -n replication-controller
NAME       DESIRED   CURRENT   READY   AGE
hello-rc   3         3         1       1m
```
### 3. check 3 pods are running
```bash
$ kubectl get pods -o wide -n replication-controller
...
### 4. delete one pod
```bash
$ kubectl delete pod hello-rc-ds5gn -n replication-controller
pod "hello-rc-ds5gn" deleted
```
### 5. Get Pods - Observe that new pod created automatically
```bash
$ kubectl get pods -w -n replication-controller
...
```bash
$ kubectl scale rc hello-rc --replicas=5 -n replication-controller
replicationcontroller/hello-rc scaled
```

### 7. Get Pods - Observer 2 new pods are created
```bash
$ kubectl get pods -n replication-controller
NAME             READY   STATUS              RESTARTS   AGE
...
```bash
$ kubectl scale rc hello-rc --replicas=0 -n replication-controller
replicationcontroller/hello-rc scaled
...
### 9. Start one replica
```bash
$ kubectl scale rc hello-rc --replicas=1 -n replication-controller
replicationcontroller/hello-rc scaled
...
$ kubectl delete rc hello-rc -n replication-controller
replicationcontroller "hello-rc" deleted

$ kubectl get all -n replication-controller
No resources found in replication-control namespace.
```